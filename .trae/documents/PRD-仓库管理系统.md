...
2. **基础资料管理** - 仓库信息、仓位信息、物资档案、供应商管理
3. **入库管理** - 采购入库、自制入库、归还退库、调拨入库、入库单查询
4. **出库管理** - 领用出库（低值易耗/展会物资）、报废出库、报损出库、出库单查询
5. **调拨管理** - 仓库间调拨、展会物资调拨、调拨单查询
6. **库存管理** - 库存查询、库存预警、盘点管理、库存流水
7. **固定资产** - 资产档案、资产领用、资产调拨、资产报废
8. **报表管理** - 库存报表、入库汇总报表、出库汇总报表
9. **反确认功能** - 支持入库单、出库单、调拨单的反确认操作
...
| 入库管理-反确认 | 反确认操作 | 对已提交/已确认的入库单执行反确认，回退到待审核状态 |
| 出库管理-反确认 | 反确认操作 | 对已提交/已确认的出库单执行反确认，回退到待审核状态 |
| 调拨管理-反确认 | 反确认操作 | 对已提交/已确认的调拨单执行反确认，回退到待审核状态 |
| 库存管理-库存流水 | 冲销流水 | 记录反确认产生的库存冲销流水，transactionType为reversal |

## 2.4 反确认功能说明

### 2.4.1 功能概述

反确认功能允许用户对已提交（submitted）或已确认（confirmed）的单据进行撤销操作，将单据状态回退到待审核（pending）状态，以便重新编辑或删除。
...
1. **反确认时机**：反确认按钮在单据状态为"已提交"或"已确认"时显示，其他状态不显示
2. **库存回滚**：已确认的单据反确认时，系统自动回滚库存数量（入库单减少库存，出库单增加库存）
3. **库存校验**：反确认回滚库存时，如果仓库配置为不允许负库存，则回滚后库存不能为负
4. **冲销记录**：已确认单据反确认会生成 `transactionType: 'reversal'` 的库存流水，关联原流水记录
