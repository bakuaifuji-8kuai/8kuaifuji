...
// 找到MOCK_CONTRACTS的结束位置（第二个];前面）
const mockContractsEnd = content.indexOf('];\n\nconst MOCK_INITIAL_ORDERS');
if (mockContractsEnd === -1) {
  console.log('Could not find MOCK_CONTRACTS end');
  process.exit(1);
}

// 在MOCK_CONTRACTS的最后一个元素后面插入新合同
...
// 找到MOCK_INITIAL_ORDERS的结束位置
const mockOrdersEnd = content.indexOf('];\n\nexport default function');
if (mockOrdersEnd === -1) {
  console.log('Could not find MOCK_INITIAL_ORDERS end');
  process.exit(1);
}

// 在MOCK_INITIAL_ORDERS的最后一个元素后面插入新订单
