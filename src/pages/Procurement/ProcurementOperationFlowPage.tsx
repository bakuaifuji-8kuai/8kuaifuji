...
const operationFlows: OperationFlow[] = [
  ...
  },
  {
    ...
    businessLogic: '需求申请可从采购计划引用或独立创建。审批通过后，需求进入采购工单生成环节。',
    steps: [
      ...
      { stepNo: 6, title: '生成采购工单', description: '已通过需求可生成采购工单', operator: '采购专员', systemAction: '点击「生成工单」，自动创建采购工单并关联需求', businessRule: '工单物资明细从需求带入', nextStep: '采购工单管理' },
    ],
    keyFields: ['需求编号', '需求类型', '项目名称', '申请部门', '物资明细', '采购申请单价', '含税金额', '审批状态'],
    statusFlow: [
      { status: 'draft', label: '草稿', color: 'text-[#909399]', description: '新增后初始状态，可编辑修改' },
      { status: 'pending', label: '待审批', color: 'text-[#e6a23c]', description: '已提交审批，等待审批人处理' },
      { status: 'approved', label: '已通过', color: 'text-[#67c23a]', description: '审批通过，可生成采购工单' },
  ...
  },
  {
    ...
    description: '管理采购工单的创建、发布、报价收集、评审、成交确认全流程。',
    businessLogic: '采购工单是采购执行的核心载体，包含物资明细、单品上限、整单上限、受邀供应商、报价对比、成交确认等关键信息。',
    steps: [
      ...
      { stepNo: 9, title: '完成工单', description: '点击「完成」，工单状态变更为「已完成」', operator: '采购专员', systemAction: '状态变更为「已完成」，记录完成时间', businessRule: '完成后可生成合同台账', nextStep: '合同台账管理' },
    ],
    ...
    relatedModules: ['采购需求申请', '供应商报价单', '合同台账管理'],
  ...
  },
  {
    ...
    businessLogic: '报价单是供应商对采购工单的响应，包含物资明细报价、税率、交货日期。系统自动判断是否超限。',
    steps: [
      ...
      { stepNo: 6, title: '回写工单', description: '点击「回写工单」，将报价信息回写到采购工单', operator: '采购专员', systemAction: '报价明细回写到工单报价记录', businessRule: '回写后工单可查看完整报价信息', output: '工单报价记录' },
    ],
    keyFields: ['报价单编号', '采购工单', '供应商', '税率', '含税金额', '税额', '是否超限', '报价状态'],
    statusFlow: [
  ...
  },
  {
    ...
    description: '管理采购订单的创建、审批、发送、变更全流程。',
    businessLogic: '采购订单可从需求申请或合同生成，包含物资明细、供应商、价格、交货日期。审批通过后发送给供应商执行。',
    steps: [
    ...
    relatedModules: ['采购需求申请', '合同台账管理', '入库管理'],
  ...
  },
  {
    ...
    businessLogic: '合同台账可从采购工单生成或独立创建，包含合同基本信息、对方单位、金额、签订日期、履行情况等。',
    steps: [
      { stepNo: 1, title: '新增合同', description: '点击「新增合同」，填写合同基本信息（合同编码、名称、分类、类型）', operator: '合同管理员', systemAction: '生成合同编号，初始化草稿状态', businessRule: '分类可选：展览服务、展览展示、采购类、招商类、其他', output: '草稿合同' },
      { stepNo: 2, title: '关联采购工单', description: '点击「选择工单」，从已完成工单中选择，自动带入项目名称、供应商信息', operator: '合同管理员', systemAction: '弹窗选择工单，自动带入信息', businessRule: '工单确认供应商自动带入对方单位', output: '关联工单信息' },
      ...
      { stepNo: 6, title: '合同执行', description: '合同进入执行阶段，记录履行情况、付款情况', operator: '合同管理员', systemAction: '状态变更为「执行中」，记录履行情况', businessRule: '执行中合同可生成采购订单', output: '执行中合同' },
    ...
    relatedModules: ['采购工单管理', '采购订单管理'],
    tips: ['关联采购工单后，项目名称、供应商信息自动带入', '合同终止为不可逆操作，请谨慎操作', '执行中合同可生成采购订单，价格从合同带入'],
  },
  {
    id: 'supplier',
    moduleName: '供应商管理',
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-[#303133]">采购管理功能操作流程</h1>
          <p className="text-xs text-[#909399] mt-1">
            本页面展示采购管理模块各功能的详细操作流程与业务逻辑说明，点击卡片查看完整流程。
          </p>
        </div>
      </div>
