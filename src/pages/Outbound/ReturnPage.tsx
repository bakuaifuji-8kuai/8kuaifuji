...
const helpContent = {
  title: '物资归还 - 功能操作说明',
  description: '物资归还用于处理已出库物资的归还入库操作。',
  sections: [
    {
      heading: '新增退库单',
      items: [
        '点击"新增退库单"按钮打开新增弹窗',
        '点击"选择来源单据"按钮，选择已出库的物资',
        '仅支持会展物资类仓库的展会领用出库单作为来源',
        '选择来源单据后，系统自动带出物资明细和仓库信息',
        '可修改实际归还数量（不能超过原出库数量）',
...
export default function ReturnPage() {
  ...
  const allSourceOrders = useMemo((): UnifiedSourceOrder[] => {
    ...
    const exhibitionWarehouseIds = new Set(filteredWarehouses.map(w => w.id));

    outboundOrders
      .filter(o => o.type === 'exhibition' && o.status === 'confirmed')
      .filter(o => exhibitionWarehouseIds.has(o.warehouseId))
      .forEach(o => {
        const exhibitionName = getOutboundExhibitionName(o) || '';
        const projectName = workOrderConfigs.find(c => c.workOrderId === o.details[0]?.workOrderId)?.exhibitionName || '';
        const implementUnit = (o as any).implementUnit || '';
        
        if (exhibitionName && isExhibitionFrozen(exhibitionName)) {
          return;
        }
    ...
    return result;
  }, [outboundOrders, filteredWarehouses, workOrderConfigs, isExhibitionFrozen]);
  ...
  }, [allSourceOrders, sourceFilterNo, sourceFilterProject]);
  ...
  const columns: ColumnDef<ReturnOrder>[] = [
    ...
    { key: 'operator', title: '操作员', render: (row) => row.operator || '-' },
    {
      ...
      render: (row) => {
        ...
        return t === 'requisition' ? '领用出库' : t === 'scrap' ? '报废出库' : t === 'damaged' ? '报损出库' : '-';
      },
    },
    {
      key: 'status',
  ...
  const handleConfirm = (id: string) => {
    ...
    (order.details as any[]).forEach((d) => {
      ...
      addStockTransaction({
        ...
        remark: (order as any).remark || '归还退库',
      });
    });
    ...
    <div className="p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-semibold text-[#303133]">物资归还</h2>
      ...
      >
        {editItem && (
          <div className="space-y-3 text-xs">
            {/* 选择来源单据 */}
            {isNew && !selectedSourceOrder && (
              <div className="border border-[#ebeef5] rounded p-3 bg-[#f5f7fa]">
                <div className="mb-3 font-medium text-[#303133]">请选择展会领用出库单</div>
            ...
            {(selectedSourceOrder || !isNew) && (
              <>
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={editItem.sourceType === 'requisition' ? '领用出库' : editItem.sourceType === 'scrap' ? '报废出库' : editItem.sourceType === 'damaged' ? '报损出库' : ''}
                      disabled
                    />
                  </div>
                  <div>
                ...
                <div className="font-medium text-[#303133] mb-2">产品明细（来源于出库单）</div>
