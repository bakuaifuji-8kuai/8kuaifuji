...
import type { ServiceApplication, ServiceApplicationDetail, ServiceApplicationStatus, Attachment } from '@/types';

const helpContent = {
  title: '服务申请审批 - 功能操作说明',
  description: '服务申请审批用于管理服务采购申请的全流程，从员工发起申请、部门审批到最终归档。系统支持新增、编辑、提交、审批、导入导出等完整操作。',
  sections: [
    {
      heading: '一、服务申请审批概述',
      items: [
        '业务定位：服务申请审批是采购管理的入口环节，员工通过此功能发起服务采购申请',
        '申请内容：每条申请包含申请人、申请部门、申请日期、期望日期、服务明细、附件等',
        '单据编号：系统自动生成以 FW 开头的申请单号，如 FW20250630001',
        '明细记录：一条申请可包含多条服务明细，记录每种服务的名称、规格、单位、申请理由',
    ...
    },
    {
      ...
      items: [
        ...
        '明细字段：服务名称（必填）、规格（选填）、单位（选填）、理由（选填）',
        '导入明细：点击「导入」按钮可从 Excel 文件批量导入服务明细',
        '模板下载：导入前请先下载标准模板，按照模板格式准备数据',
        '删除明细：点击明细行右侧的删除按钮移除该条明细',
        '明细要求：至少需要添加一条服务明细才能提交申请'
      ]
    },
    {
      heading: '五、提交与保存草稿',
    ...
    },
    {
      ...
      items: [
        ...
        '服务明细：以表格形式展示所有服务明细，包含名称、规格、单位、理由',
    ...
    },
    {
      heading: '八、导入导出功能',
      items: [
        '导入操作：点击「导入服务」按钮，选择 Excel 文件（.xlsx 或 .xls）',
        '导入范围：导入功能用于批量添加服务明细到当前申请中',
...
const statusBadgeVariant = (status: ServiceApplicationStatus) => {
  switch (status) {
...
export default function ServiceApplicationPage() {
  ...
  const [viewItem, setViewItem] = useState<ServiceApplication | null>(null);
  const [importModalOpen, setImportModalOpen] = useState(false);
  const [importPreview, setImportPreview] = useState<ServiceApplicationDetail[]>([]);
  ...
  // 申请明细
  const [details, setDetails] = useState<ServiceApplicationDetail[]>([]);
  ...
  const filteredData = useMemo(() => {
    return serviceApplications.filter((item) => {
  ...
  }, [serviceApplications, applied]);

  // 表格列定义
  const columns: ColumnDef<ServiceApplication, unknown>[] = [
  ...
  // 编辑草稿
  const handleEdit = (item: ServiceApplication) => {
    setIsNew(true);
    setFormData({
  ...
  const handleSubmit = () => {
    ...
    if (isNew) {
      const newApp: ServiceApplication = {
        ...
        details: details.map(d => ({ ...d, id: 'SAD' + Date.now() + Math.random().toString(36).slice(2, 6) })),
        attachments: attachments,
        createTime: new Date().toISOString().slice(0, 19),
      };
      addServiceApplication(newApp);
    }
  ...
  // 审批点击
  const handleApproveClick = (item: ServiceApplication, status: 'approved' | 'rejected') => {
    setViewItem(item);
  ...
  const handleApproveConfirm = () => {
    ...
    updateServiceApplication(viewItem.id, {
  ...
  // 更新明细行
  const handleUpdateDetail = (index: number, field: keyof ServiceApplicationDetail, value: string | number) => {
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      { '服务名称': '', '规格型号': '', '单位': '', '申请理由': '' },
      { '服务名称': 'LED显示屏', '规格型号': 'P2.5高清', '单位': '平方米', '申请理由': '展会项目需要' },
      { '服务名称': '音响设备', '规格型号': '专业级', '单位': '套', '申请理由': '展会使用' },
    ];

    const ws = XLSX.utils.json_to_sheet(templateData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, '服务申请导入模板');
    ws['!cols'] = [
      { wch: 20 }, { wch: 20 }, { wch: 10 }, { wch: 20 },
    ];
    XLSX.writeFile(wb, '服务申请导入模板.xlsx');
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const previewData: ServiceApplicationDetail[] = [];
        jsonData.forEach((row, index) => {
          const name = row['服务名称'];
          if (!name) return;

          previewData.push({
            id: 'SAD' + Date.now() + index,
            applicationId: '',
            productName: name,
  ...
  return (
    <div className="p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-semibold text-[#303133]">服务申请审批</h2>
      ...
      {/* 新增/编辑弹窗 */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="服务申请" size="max-w-4xl">
        <div className="space-y-4 min-w-[700px]">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-slate-700">申请服务明细</label>
      ...
      <Modal open={!!viewItem} onClose={() => setViewItem(null)} title="申请详情" size="max-w-4xl">
        {viewItem && (
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-slate-700 mb-2 block">申请服务明细</label>
              <div className="border border-slate-200 rounded overflow-hidden">
                <table className="w-full text-xs">
                  <thead className="bg-slate-50">
                    <tr>
      ...
      <Modal open={approveModal} onClose={() => setApproveModal(false)} title={approveStatus === 'approved' ? '审批通过' : '审批驳回'} size="max-w-md">
        <div className="space-y-4">
          <div className="text-sm text-slate-600">
            确认{approveStatus === 'approved' ? '通过' : '驳回'}此服务申请单？
          </div>
          <Input
            label="审批意见"
            value={approveRemark}
      ...
      >
        <div className="space-y-4">
          <div className="text-sm text-slate-600">
            共 <span className="font-semibold text-blue-600">{importPreview.length}</span> 条服务待添加
          </div>
